...
async def share_user(message:types.Message):
    await message.answer(f"{message.from_user.full_name}вы можете поделится своими данными ",share_markup=share_button)

@dp.message_handler(content_types=types.ContentType.CONTACT)
async def get_contact(message:type.Message):
    cursor= connect.cursor()
    cursor.execute(f"UPTADE users SET phone= '{message.contact['phone_number']}' WHERE id= {message.from_user_id};")
    connect.commit()
    await message.answer("Ваш контакный номер записан")
